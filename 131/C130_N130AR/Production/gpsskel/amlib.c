...
static NR_TYPE
		UPFCTR	= 0.999444,
		FCTRF	= 0.997,
/*		TAUP    = 100.0,      <-- MAP used 600.0 in Defaults file  */
		TAUP    = 600.0,
		CDM	= 111120.0,
/*		ROLL_MAX = 40.0;      <-- MAP used  24.0 in Defaults file  */
		ROLL_MAX = 24.0;
...
/*    Diagnostic prints begin  */
    if (diag)
    {
      printf ("Changed equations originally on lines 494-497.\n Printing values that are now ignored in 'no good' GPS calculations.\n  (See lines 506-509.)\n");
    }
...
/*   The code should be (if translated from the original FORTRAN version):
              c[k][i] += hi[j][i] * am[k][j];
...
*/
/*    Try new stuff here
      dvy[FeedBack]  *= FCTRF+(1-FCTRF)*(c[0][0]+c[0][1]*sinwt+c[0][2]*coswt);
      dvx[FeedBack]  *= FCTRF+(1-FCTRF)*(c[1][0]+c[1][1]*sinwt+c[1][2]*coswt);
      dlat[FeedBack] *= FCTRF+(1-FCTRF)*(cp[0][0]+cp[0][1]*sinwt+cp[0][2]*coswt);
      dlon[FeedBack] *= FCTRF+(1-FCTRF)*(cp[1][0]+cp[1][1]*sinwt+cp[1][2]*coswt);
  */
