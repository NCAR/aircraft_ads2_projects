...
NR_TYPE	(*pcorPSF)(NR_TYPE), (*pcorQCF)(NR_TYPE), (*pcorPSFD)(NR_TYPE),
	...
	(*pcorPSB)(NR_TYPE), (*pcorQCB)(NR_TYPE), (*pcorQCFR)(NR_TYPE);
...
/*  Zap this code until the flight date values come in correctly */
      sprintf(buffer, "%04d%02d", FlightDate[2], FlightDate[0]);
printf("Flight date: %d %d %d\n", FlightDate[2], FlightDate[1], FlightDate[0]);
printf("buffer contents: %s compared to 200309\n",buffer);
/*  */
...
printf(">>>>>>>>>>>>>>>  AIRS-II and later pcors\n");
        pcorPSF	= pcorf1_3;
        pcorQCFR = pcorf1_4;
/*  This version to use Pre-AIRS-II pcors  */
      }
      else
      { /* Pre-AIRS-II */
...
NR_TYPE pcorf1_4(NR_TYPE q)	/* For new QCFR */
{
  NR_TYPE	pcor;
  
  pcor = 10.65 + q * ( -0.1891 + 0.001397 * q);
...
NR_TYPE pcorf1_3(NR_TYPE q)	/* For new PSF */
{
  NR_TYPE	pcor;
