...
Widget parent;
{
	...
	for (i = 0; i < 2; ++i)
		{
		sprintf(buffer, "FNform%d", i);
		form[i] = XmCreateForm(fileNameRC, buffer, args, n);
		}
	...
	XtSetArg(args[n], XmNrightWidget, aDSdataText); n++;
	XtManageChild(XmCreateLabel(form[0], "FNlabel0", args, n));
	XtSetArg(args[4], XmNrightWidget, outputFileText);
...
/*	XtManageChild(pullRight); */
	...
	b[4] = XmCreateCascadeButton(pullDown[1], "toggleProbe", args, n);
...
/*	XtAddCallback(b[4], XmNactivateCallback, ToggleProbeMenu, NULL); */
	...
	XtSetArg(args[n], XmNlabelString, (xmstr[0]=XmStringCreateLtoR(
		"  Name     Output SR(hz) OR(hz) Lag(ms)  Cal Coes/Dependencies",
...
Widget	parent;
{
	...
	frame[4] = XmCreateFrame(editForm, "buttonFrame", args, n);
	n = 0;
	XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET); n++;
	XtSetArg(args[n], XmNtopWidget, frame[0]); n++;
	XtSetArg(args[n], XmNbottomAttachment, XmATTACH_WIDGET); n++;
	XtSetArg(args[n], XmNbottomWidget, frame[4]); n++;
	XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
	XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET); n++;
	XtSetArg(args[n], XmNleftWidget, frame[1]); n++;
	frame[5] = XmCreateFrame(editForm, "evTextFrame1", args, n);
/*
	n = 0;
	XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM); n++;
	XtSetArg(args[n], XmNbottomAttachment, XmATTACH_WIDGET); n++;
	XtSetArg(args[n], XmNbottomWidget, frame[4]); n++;
	XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET); n++;
	XtSetArg(args[n], XmNleftWidget, frame[5]); n++;
	XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
	frame[6] = XmCreateFrame(editForm, "evTextFrame2", args, n);
*/
	 ...
	 */
	n = 0;
	cbRC = XmCreateRowColumn(frame[4], "buttonRC", args, n);
	XtManageChild(cbRC);
	...
	n = 0;
	evRC[0] = XmCreateRowColumn(frame[5], "evTextRC1", args, n);
	XtManageChild(evRC[0]);
/*	evRC[1] = XmCreateRowColumn(frame[6], "evTextRC2", args, n);
	XtManageChild(evRC[1]);
*/
...
/*		XtAddCallback(ev_text[i], XmNactivateCallback,
				(XtCallbackProc)XmProcessTraversal, XmTRAVERSE_NEXT_TAB_GROUP);
*/		}
/*
	for (; i < 16; ++i)
		{
		sprintf(buff, "ev_text%d", i);
		ev_text[i] = XmCreateTextField(evRC[1], buff, args, n);
...
Widget	parent;
{
	...
	n = 0;
	form = XmCreateForm(parent, "timeSliceForm", args, n);
   ...
   Start      Finish", XmSTRING_DEFAULT_CHARSET))); n++;
	label = XmCreateLabel(form, "TSlabel", args, n);
	...
	XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM); n++;
	XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
	tsRC = XmCreateRowColumn(form, "timeSliceRC", args, n);
	XtManageChild(tsRC);
