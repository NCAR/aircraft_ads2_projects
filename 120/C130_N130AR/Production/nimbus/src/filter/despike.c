...
static int		nsdi_spike = 0, nraw_spike = 0, print;

static void checkVariable(int SRstart, int rate, NR_TYPE slope);
...
void AddVariableToSDIdespikeList(SDITBL *varp)
{
	...
	sprintf(buffer, "Despike for %s enabled, with slope of %f.\n",
					varp->name, varp->SpikeSlope);
	LogMessage(buffer);
...
/* -------------------------------------------------------------------- */
static void checkVariable(int SRstart, int SampleRate, NR_TYPE SpikeSlope)
{
