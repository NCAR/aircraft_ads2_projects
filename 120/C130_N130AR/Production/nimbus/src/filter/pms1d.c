...
void SetUpPMS1D_SHMEM()
{
	...
	for (i = 0; i < pcnt; ++i)
		{
		strcpy(PMS1Dprobe[i].name, probe_name[i]);
		...
		if (PMS1Dprobe[i].type == FSSP)
			...
			strcat(target, strchr(PMS1Dprobe[i].name, '_'));
			strcpy(PMS1Dprobe[i].rangeName, target);
...
int		length;
{
	...
	if (indx == pcnt)
		{
		strcpy(PMS1Dprobe[pcnt].name, &name[1]);
	...
	switch (name[0])
		{
		case 'A':
			return(PMS1Dprobe[indx].actual);
			break;

		case 'C':
			return(PMS1Dprobe[indx].conc);
			break;
		}
