...
EditVariable(Widget w, XtPointer client, XmListCallbackStruct *call)
{
	...
	switch (VariableType)
	  ...
	  case SDI:
		{
		set_edit_window_data(sp->name,sp->Output,sp->OutputRate,sp->StaticLag);
	  ...
	  case RAW:
		{
		set_edit_window_data(rp->name,rp->Output,rp->OutputRate,rp->StaticLag);
	  ...
	  case DERIVED:
		{
		set_edit_window_data(dp->name, dp->Output, dp->OutputRate, ERR);
...
/* -------------------------------------------------------------------- */
static void set_edit_window_data(name, output, outputRate, lag)
char	name[];
bool	output;
int		outputRate;
int		lag;
