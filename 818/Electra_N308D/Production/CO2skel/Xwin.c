...
Widget parent;
{
	...
	n = 0;
	quitButton = XmCreatePushButtonGadget(buttonRC, "quitButton", args, n);
	XtAddCallback(quitButton, XmNactivateCallback, Quit, NULL);
	XtManageChild(quitButton);

	n = 0;
	readHeaderButton = XmCreatePushButtonGadget(buttonRC, "proceedButton", args, n);
	...
	XtSetArg(args[n], XmNrightWidget, inputFileText); n++;
	XtManageChild(XmCreateLabelGadget(form[0], "FNlabel0", args, n));
	XtSetArg(args[4], XmNrightWidget, outputFileText);
	XtManageChild(XmCreateLabelGadget(form[1], "FNlabel1", args, n));
...
Widget	parent;
{
	...
	n = 0;
	b[0] = XmCreatePushButtonGadget(pullDown[0], "cancelSetupButton", args, n);
    XtAddCallback(b[0], XmNactivateCallback, CancelSetup, NULL);
	XtManageChild(b[0]);

	n = 0;
	b[0] = XmCreatePushButtonGadget(pullDown[1], "editTimeSlices", args, n);
	b[1] = XmCreatePushButtonGadget(pullDown[1], "toggleOutput", args, n);
    ...
    goButton = XmCreatePushButtonGadget(setupForm, "goButton", args, n);
	...
	listTitle = XmCreateLabelGadget(listRC, "listTitle", args, n);
...
Widget	parent;
{
   ...
   Start      Finish", XmSTRING_DEFAULT_CHARSET))); n++;
	label = XmCreateLabelGadget(form, "TSlabel", args, n);
