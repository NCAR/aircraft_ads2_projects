...
char	file_name[];
{
	...
	int			ndims, dims[3],
				...
				Vector16Dim, Vector32Dim, Vector64Dim;
	int			j, indx;
	char		*p;
	...
	ncattput(OutputFile,NC_GLOBAL,"Source",NC_CHAR,strlen(buffer)+1,(void*)buffer);

	strcpy(buffer, "NCAR-RAF/nimbus");
	ncattput(OutputFile, NC_GLOBAL, "Conventions", NC_CHAR,
...
long	nRecs;
{
	...
	otoid = ncvarid(InputFile, "time_offset");
